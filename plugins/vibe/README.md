...
| 순서 | 스킬 | 산출물 |
|---|---|---|
| 0 | `$vibe:00-start` | `docs/setup.md` (대화 방식·아이디어) |
...
```text
$vibe:00-start 만들고 싶은 서비스 아이디어
```

0단계에서 AI와의 대화 방식을 고릅니다 — 간단 모드는 핵심 결정만 묻고 나머지는 추천으로 채우며, 꼼꼼 모드는 결정을 하나씩 배우며 진행합니다. 완료 기준은 로컬 실행이며, 배포는 계획의 마지막 "배포(선택)" EPIC으로 분리됩니다.
