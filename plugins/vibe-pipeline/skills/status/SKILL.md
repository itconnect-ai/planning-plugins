...
1. 전체 흐름에서 현재 위치: `1 PRD → 2 디자인 → 3 FRD → 4 TRD → 5 계획 → 개발` 중 어디까지 완료됐는지 (각 단계 산출물 prd.md, design-prompt.md, frd.md, trd.md, plan.md의 존재와 상태 줄로 판단. "상태: 작성 중"이면 그 단계는 미완료 — 해당 명령 재실행을 안내. 상태 줄이 없는 기존 문서는 완료로 간주. setup.md는 1단계가 만드는 시작 설정 파일이므로 단계 판단에서 제외. 2단계 완료 기준은 design-prompt.md이며 design/ 이미지는 선택 산출물이라 없어도 다음 단계를 막지 않는다 — 없으면 "시안 이미지: 아직 없음(선택)"으로만 표시)
